...
export class TLBWalk {
  ...
  constructor(container: HTMLElement) {
    ...
    for (let i = 0; i < 4; i++) {
         const rect = new AnimatedRect(`pt-${i}`, 250 + (i * 150), 100 + (i * 50), 100, 50);
         rect.label = levels[i];
         ...
         if (i > 0) {
             ...
             const arrow = new Arrow(`arr-${i}`, prev.x + prev.width, prev.y + prev.height / 2, rect.x, rect.y + rect.height / 2);
  ...
  private simulateWalk() {
     ...
     const walkStep = () => {
         ...
         if (currentLevel < 4) {
             this.pageTable[currentLevel].fillColor = themes['dark-terminal'].accent1;
