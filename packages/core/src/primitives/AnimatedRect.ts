...
export class AnimatedRect extends Entity {
  ...
  public label: string;
  public labelColor = "#111"; // High contrast text on gray bg by default
  public textColorOverride: string | null = null;
  public labelAlign: "center" | "left" | "right" = "center";
  /** Label font size in CSS pixels */
  public labelFontPx = 14;

  public velocity = 0; // if > 0, use constant velocity mode (px/sim-ms)
  ...
  public moveTo(x: number, y: number, velocity = 0) {
    this.targetX = x;
    this.targetY = y;
    if (velocity > 0) {
  ...
  draw(ctx: CanvasRenderingContext2D): void {
    ...
    if (this.label) {
      ...
      ctx.font = `${this.labelFontPx}px "JetBrains Mono", "Courier New", monospace`;
