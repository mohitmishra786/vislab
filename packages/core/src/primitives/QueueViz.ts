...
export class QueueViz extends Entity {
  public x: number;
  public y: number;
  public itemWidth: number = 35;
  public itemHeight: number = 35;
  public capacity: number = 5;
  public trackTint: string = 'rgba(255,255,255,0.02)';
  public trackBorder: string = 'rgba(255, 255, 255, 0.1)';
  ...
  public enqueue(label: string, color: string) {
    ...
    const rect = new AnimatedRect(`${this.id}-item-${this.items.length}`, startX, this.y - 15, this.itemWidth, this.itemHeight);
    rect.label = label;
